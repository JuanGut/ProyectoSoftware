<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="icon" type="img/png" href="data:image/png;base64," >
    <link rel="stylesheet" href="../Estilos/cabecera.css">
    <link rel="stylesheet" href="../Estilos/EstilosComp3/modulo2.css">
    <title>Contenido competencias</title>
</head>
<body>
    <nav id="Nav">
        <div id="Contenedor">
            <img src="../Recursos/iconoU.png" id="u" class="img1">
            <h1  class="titulo1"> Plataforma Industria 4.0 </h1>
            
        </div>
    </nav>

    <nav class="menu1">

        <ul class="competencias">
            <li><a href="http://localhost/Proyecto/contenidoCompetencias.php" class="comp1">Conocimiento</a></li>
            <li><a href="http://localhost/Proyecto/CompetenciasDigitales/conocimientoDigital.php" class="comp2">Competencias Digitales</a></li>
            <li><a href="http://localhost/Proyecto/HabilidadesBlandas/pensamientoCritico.php" class="comp3">Habilidades Blandas</a></li>
        </ul>
    </nav>

    <nav class="submenu">

        <ul class="modulos">
            <li><a href="http://localhost/Proyecto/HabilidadesBlandas/pensamientoCritico.php" class="mod1">Pensamiento critico</a></li>
            <li><a href="http://localhost/Proyecto/HabilidadesBlandas/pensamientoAnalitico.php" class="mod2">Pensamiento analitico</a></li>
            <li><a href="http://localhost/Proyecto/HabilidadesBlandas/solucionProblemas.php" class="mod3">Solucion de problemas</a></li>
            <li><a href="http://localhost/Proyecto/HabilidadesBlandas/inteligenciaEmocional.php" class="mod4"> Inteligencia emocional</a></li>
        </ul>
    </nav>
    <p>Lorem ipsum, dolor sit amet consectetur adipisicing elit. Tempora quis in nostrum doloremque obcaecati. Perferendis dignissimos aliquam ipsa molestias illum fuga porro 
        temporibus nisi autem voluptas debitis, saepe velit repellendus.Lorem ipsum dolor sit amet, consectetur adipisicing elit. Quas consequuntur ratione modi sit officia, labore doloremque error, perspiciatis deleniti perferendis ducimus. Consequatur doloribus cumque tempora dignissimos rem beatae deserunt harum?</p>
    <br>
    <img src="../Recursos/iconoImagen.png" alt="" class="icono">
    <br>
    <h1 id="ma">Material de apoyo</h1> 
    
    <table class="tabla" cellspacing="0" cellpadding="0">
        <thead>
            <tr>
            <th><span>Nombre Documento</span></th>
            <th><span>Link de descarga</span></th>
            </tr>
        </thead>
        <tbody>

            <tr>
            <td class="align-left">nombre1</td>
            <td><a href="">link1</a></td>
            </tr>
            <tr>
            <td class="align-left">nombre2</td>
            <td><a href="">link2</a></td>
            </tr>
            <tr>
            <td class="align-left">nombre3</td>
            <td><a href="">link3</a></td>
            </tr>
        </tbody>
    </table>
    <br>
    <h2>Encuesta de evaluacion de competencias <a href="">Link encuesta</a></h2>
    
</body>
</html>