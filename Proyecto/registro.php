<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="icon" type="img/png" href="data:image/png;base64," >
    <link rel="stylesheet" type="text/css" href="Estilos/cabecera.css">
    <link rel="stylesheet" type="text/css" href="Estilos/registro.css">
    <title>Inicio sesion</title>
</head>
<body>

    <nav id="Nav">
        <div id="Contenedor">
            <img src="Recursos/iconoU.png" id="u" class="img1">
            <h1  class="titulo1"> Plataforma Industria 4.0 </h1>
        </div>
    </nav>
    <br>
    </br>
   
    <form action="validar.php" method="post" id="inicio">
    <h1>Registro</h1>
    <p>Cedula<input type="text" placeholder="Ingrese su numero de cedula" name="cedula"></p>
    <p>Nombre<input type="text" placeholder="Ingrese su nombre" name="nombre"></p>
    <p>Apellido<input type="text" placeholder="Ingrese su Apellido" name="apellido"></p>
    <p>Contraseña<input type="text" placeholder="Ingrese su contraseña" name="contrasenia"></p>
    <p>Confirme contrasenia<input type="text" placeholder="Confirme su contraseña" name="confirmacionContrasenia"></p>
    <input type="submit" value="Registrarse">
    <br></br>
    <p id="p1">¿Ya tienes una cuenta? <a href="login.php" id="login"> Iniciar sesion</a></p>
    
    
    </form>
    

</body>
</html>